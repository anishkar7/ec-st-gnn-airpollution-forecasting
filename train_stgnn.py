...
# 2. Reshape into spatio-temporal array: [time steps, nodes, features]
...
for i in range(total_timesteps - 16 - 4 + 1):
    ...
    # Target the PM2.5 feature (index 0) for the next 4 timesteps
...
# 4. Initialize model and training hyperparameters
...
for epoch in range(epochs):
    ...
    # Forward, compute loss, and backpropagate
    ...
    # Evaluate on the validation set (no gradients)
    model.eval()
    with torch.no_grad():
...
# 6. Plot and save loss curves (publication-ready)
